/**
 * Standalone module for reading minesweeper.online's DOM.
 *
...
// ============================================================================
// MinesweeperOnlineSite — DOM-reading class for minesweeper.online
// ============================================================================

/**
 * Reads game state, board configuration, settings, and game-end events from
 * minesweeper.online's DOM. Purely passive — only reads, never writes.
 *
 * Typical usage:
 *
 ...
 *     const mines = site.getMinePositions(result)
 *     // ... build RecordingData and write RAWVF
 *   })
 *
 *   site.initSettingsBridge((settings) => {
 *     // settings changed in the site's localStorage
 *   })
 */
export class MinesweeperOnlineSite {
  private faceObserver: MutationObserver | null = null
  private resetObserver: MutationObserver | null = null
  private boardChangeObserver: MutationObserver | null = null

  // Settings polling state
  private settingsPollInterval: ReturnType<typeof setInterval> | null = null
  private cachedLocalStorageSettings: GameSettings | null = null
  ...
  getMinePositions(result?: GameResult): BoardPosition[] {
    ...
    // Always re-read the skin prefix from the DOM. This is called once per
    // game end, so the trivial DOM query cost avoids stale-cache bugs after
    // SPA navigation or skin changes.
  ...
  onBoardChange(callback: () => void): void {
    ...
    this.boardChangeObserver = new MutationObserver(() => {
      // Read-only: wait one frame for new cells to fully render.
